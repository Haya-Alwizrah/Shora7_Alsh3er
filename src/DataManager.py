...
class DataManager:
    def __init__(self, data_source, db_path, collection_name, EMBEDDING_MODEL):
        """
        data_source: يمكن أن يكون مسار مجلد أو قائمة بملفات محددة.
        """
        self.client = chromadb.PersistentClient(path=db_path)
        self.collection_name = collection_name
        self.embed_model = EMBEDDING_MODEL
        
        # التأكد من جلب جميع ملفات .txt إذا كان المدخل مجلداً
    ...
    def get_embedding_func(self):
        if self.embedding_func is None:
            # استخدام موديل التضمين المحدد في ملف البيئة
    ...
    def split_poem_blocks(self, text):
        # تقسيم النص بناءً على الفواصل الطويلة (متوافق مع ملفات المعلقات)
        chunks = re.split(r"\n[-]{10,}\n", text)
        return [chunk.strip() for chunk in chunks if chunk.strip()]
    ...
    def add_documents(self, batch_size=64):
        ...
        for i in range(0, len(self.chunks), batch_size):
            batch_chunks = self.chunks[i:i+batch_size]
            batch_metadatas = self.metadatas[i:i+batch_size]
            # توليد معرفات فريدة بناءً على اسم الملف والمعرف
    ...
    def prepare(self):
        # التحقق من وجود البيانات مسبقاً لتجنب التكرار
        if self.db_exists():
            print("DB already exists → Loading existing collection.")
            self.create_collection()
            # إذا كانت الكولكشن فارغة رغم وجودها، نقوم بالتحميل
